<?php
	...
	class XuatThuoc {
		...
		public function list_xuat_thuoc($db) {
			$query = "SELECT xuatthuoc.*, loaithuoc.TenThuoc, loaibenh.TenBenh, hang.TenHang FROM xuatthuoc INNER JOIN loaithuoc ON xuatthuoc.idLoaiThuoc = loaithuoc.id INNER JOIN loaibenh ON loaithuoc.idLoaiBenh = loaibenh.id INNER JOIN hang ON loaithuoc.idHang = hang.id";
			$statement = $db->prepare($query);
			$statement->execute();
